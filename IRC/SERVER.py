...
class WhatsUpServer(threading.Thread):
    ...
    def login(self):
        ...
        if self.addr not in accounts:
            ...
            while 1:
                name = self.conn.recv(BUF_SIZE).decode('UTF-8').strip()
                if name in messages:
                    self.print_indicator(
                        '## This name already exists, please try another')
                else:
                    break
            accounts[self.conn]['name'] = name
            self.name = name
            logging.info('%s logged as %s' % (self.addr[0], self.name))
            messages[name] = []
            self.print_indicator(
                '## Hello %s, please enter your password:' % (self.name,))
            password = self.conn.recv(BUF_SIZE).decode('UTF-8')
            accounts[self.conn]['pass'] = password.strip()
            self.print_indicator('## Welcome, enjoy your chat')
        else:
            self.name = accounts[self.conn]['name']
            msg += '## Hello %s, please enter your password:' % (self.name,)
            # print accounts
            self.print_indicator(msg)
            while 1:
                password = self.conn.recv(BUF_SIZE).decode('UTF-8').strip()
                if password != accounts[self.conn]['pass']:
                    self.print_indicator(
                        '## Incorrect password, please enter again')
                else:
                    self.print_indicator(
                        '## Welcome back, last login: %s' %
                        (accounts[self.conn]['lastlogin'],))
                    accounts[self.conn]['lastlogin'] = time.ctime()
                    break
            self.conn.send(bytes(self.show_mentions(self.name),'UTF-8'))
    ...
    def broadcast(self, msg, receivers, to_self=True):
        for conn, addr in receivers:
            ...
            # if current user
                ...
                self.conn.send(b'>> ') if to_self else self.conn.send(b'')
    ...
    def run(self):
        ...
        while 1:
            try:
                ...
                buf = self.conn.recv(BUF_SIZE).decode('UTF-8').strip()
